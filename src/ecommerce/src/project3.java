...
public class project3 {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        // Create an array of WebDriver instances
        WebDriver[] drivers = new WebDriver[2];  // Adjust the number based on the desired parallelism

        // Create and start each WebDriver instance
        for (int i = 0; i < drivers.length; i++) {
            final int index = i;

            if (index % 2 == 0) {
                // System.setProperty("http.client", "jdk");
                drivers[index] = new ChromeDriver();
            } else {
                // System.setProperty("http.client", "jdk");
                drivers[index] = new EdgeDriver();
            }


            // Start parallel execution for each WebDriver instance
            new Thread(() -> {
                try {
                    executeTest(drivers[index]);
        ...
        // Wait for all threads to finish before quitting the drivers
    ...
    private static void executeTest(WebDriver driver) throws CsvValidationException {
        try {
            cookies(driver);
            Thread.sleep(2000);
